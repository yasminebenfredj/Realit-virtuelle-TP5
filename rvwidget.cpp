...
void RVWidget::initializeGL()
{
    ...
    dice->setPosition(QVector3D(10, 2, 0));
    dice->initialize();
    dice->setScale(2);
    dice->setObjectName("Dé1");
    dice->setTexture(":/textures/3d-cube-textures/dice_texture.jpg");

    scene.append(dice);
    camera->setTarget(dice->getPosition());

    // plane
    plane = new RVPlane(40, 40);
    plane->setCamera(camera);
    plane->setPosition(QVector3D(0, -5, 0));
    ...
    specCube->setPosition(QVector3D(7, 5, 7));
    specCube->initialize();
    specCube->setScale(3);
...
{
    scene.at(selectedObject)->setSpecStrength((float)val);
    QOpenGLWidget::update();
}
...
///
/// \brief RVWidget::startAllAnimation
///
void RVWidget::startAllAnimation()
{
    animationAllOn = !animationAllOn;
...
///
/// \brief RVWidget::getRotateSoleil
/// \return
///
...
///
/// \brief RVWidget::setRotateSoleil
/// \param newRotateSoleil
///
...
const QTime &RVWidget::getStartTime() const
{
    return startTime;
}

void RVWidget::setStartTime(const QTime &newStartTime)
{
    startTime = newStartTime;
}

const QTime &RVWidget::getLastTime() const
{
    return lastTime;
}

void RVWidget::setLastTime(const QTime &newLastTime)
{
    lastTime = newLastTime;
}

RVCurve *RVWidget::getTrajectory() const
{
    return trajectory;
}

void RVWidget::setTrajectory(RVCurve *newTrajectory)
{
